...
import { useGroup, useGroupMembers, useGroupStats, useMakeContributionWithToken, useGroupContributions, useGroupMilestones } from "@/hooks/useTsaroSafe";
...
export default function GroupDetailPage() {
  ...
  const { makeContributionWithToken, isLoading: isSubmitting, isConfirmed, error: contributionError } = useMakeContributionWithToken();
          ...
          <div className="mb-4">
            ...
            <div className="flex justify-between text-sm text-gray-500 mt-2">
              <div className="text-left font-bold text-gray-900">
                {currentAmount.toLocaleString('en-US', { maximumFractionDigits: 2 })} CELO
              </div>
              <div className="text-right font-bold text-gray-900">
                {targetAmount.toLocaleString('en-US', { maximumFractionDigits: 2 })} CELO
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="p-3 border rounded-lg bg-gray-50">
                    <div className="inline-flex items-center px-3 py-2 rounded text-sm font-medium bg-yellow-100 text-yellow-800">
                      CELO
                    </div>
                    <p className="text-xs text-gray-500 mt-1">
                      This group accepts contributions in CELO only.
                    </p>
                  </div>
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-600 focus:outline-none text-gray-900 bg-white"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-bold text-gray-900">
                          {(Number(contribution.amount) / 1e18).toLocaleString('en-US', { maximumFractionDigits: 2 })} CELO
                        </p>
                        <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-yellow-100 text-yellow-800">
                          CELO
                        </span>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          Target: {(Number(milestone.targetAmount) / 1e18).toLocaleString('en-US', { maximumFractionDigits: 2 })} CELO
                        </p>
                      </div>
                      {milestone.isReached && (
