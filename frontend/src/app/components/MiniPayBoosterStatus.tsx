...
export default function MiniPayBoosterStatus() {
  const { isMiniPay, isMiniPayConnected, minipayBalance, isConnecting, connectError, autoConnectMiniPay } = useMiniPay();
    ...
    <div className="w-full bg-gradient-to-r from-amber-500/10 via-yellow-500/20 to-amber-500/10 border-b border-yellow-500/30 py-1.5 px-3 flex justify-between items-center text-[10px] relative overflow-hidden transition-all duration-300">
      ...
      <div className="flex items-center gap-2 relative z-10">
        {isMiniPayConnected ? (
          ...
          <button 
            onClick={() => autoConnectMiniPay?.(true)} 
            className="text-[9px] font-bold text-red-400 hover:text-red-300 uppercase tracking-wider underline cursor-pointer bg-transparent border-none p-0 outline-none"
          >
          ...
          <button 
            onClick={() => autoConnectMiniPay?.(true)} 
            className="text-[9px] font-bold text-yellow-500 hover:text-yellow-400 uppercase tracking-wider underline cursor-pointer bg-transparent border-none p-0 outline-none"
          >
            Connect Wallet
          </button>
        )}
