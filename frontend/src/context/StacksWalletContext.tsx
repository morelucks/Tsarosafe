...
type StacksNetwork = typeof STACKS_MAINNET | typeof STACKS_TESTNET | typeof STACKS_DEVNET;
...
export function StacksWalletProvider({ children }: { children: React.ReactNode }) {
  ...
  // Stacks network mapping (v7 uses constant objects, not class constructors)
  const getNetworkConfig = (name: 'mainnet' | 'testnet' | 'devnet'): StacksNetwork => {
    switch (name) {
      case 'testnet':
